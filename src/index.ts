...
async function filaEnfileirarLote(
  ...
  for (let i = 0; i < itens.length; i += CHUNK) {
    const grupo = itens.slice(i, i + CHUNK).map((it) =>
      env.DB.prepare(
        `INSERT OR IGNORE INTO fila_trabalho (chave, tipo, agenda_id_cnn, paciente_id_cnn, grupo, payload, status, tentativas, criado_em, atualizado_em)
         VALUES (?, ?, ?, ?, ?, ?, 'pendente', 0, ?, ?)`
...
async function registrarLembrete(
  ...
  await env.DB.prepare(
    `INSERT OR IGNORE INTO lembrete_d1 (chave, lead_id_kommo, agenda_id_cnn, data_agendamento, grupo, pipeline_destino, etapa_destino, enviado_em)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
